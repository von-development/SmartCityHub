...
export function TrafficIncidentsSection() {
  const { showIncidents, setShowIncidents } = useTraffic()
  const { data: trafficIncidents, isLoading } = useTrafficIncidents(null)
  const incidents = trafficIncidents?.incidents || []
              ...
              incidents.map((incident) => (
                <Card key={incident.id} className="p-3">
                  <div className="flex items-start gap-3">
                    <AlertTriangle className={`h-4 w-4 flex-shrink-0 ${getSeverityColor(incident.severity)}`} />
